...
assert_address_contract( true, false !== strpos( $shipping_zones_source, "wp_add_inline_script( 'wc-shipping-zone-methods'" ) && false !== strpos( $shipping_zones_source, "'continent:AS'" ) && false !== strpos( $shipping_zones_source, "'country:VN'" ) && false !== strpos( $shipping_zones_source, "'state:VN:' + provinceCode" ), 'Ward options are attached to the native Asia, Vietnam, and Province / City hierarchy before the picker mounts' );
assert_address_contract( true, false !== strpos( $shipping_zones_source, "locationType + ':' + provinceCode + ':' + wardCode" ) && false !== strpos( $shipping_zones_source, "wardName.trim() + ', ' + provinceName.trim()" ), 'Native ward nodes use stable values and unambiguous Ward / Commune, Province / City labels' );
assert_address_contract( true, false !== strpos( $shipping_zones_source, 'existingValues[value]' ) && false !== strpos( $shipping_zones_source, 'plannedValues[value]' ) && false !== strpos( $shipping_zones_source, 'Array.prototype.push.apply(plan.state.children, plan.additions)' ), 'Native tree augmentation is idempotent and appends ward nodes without replacing existing child arrays' );
assert_address_contract( true, false !== strpos( $shipping_zones_source, 'plans.length !== provinceCodes.length' ) && false !== strpos( $shipping_zones_source, 'provinceCodes.length !== wardProvinceCodes.length' ) && false !== strpos( $shipping_zones_source, "1 !== state.length" ), 'Native tree augmentation validates the complete state/data structure before applying its mutation plan' );
assert_address_contract( true, false === strpos( $shipping_zones_source, 'data.region_options =' ) && false === strpos( $shipping_zones_source, 'data.locations =' ) && false === strpos( $shipping_zones_source, 'ShippingZoneInitialWards' ), 'Structural failure and initial restoration preserve WooCommerce native options and localized locations' );
assert_address_contract( true, false === strpos( $shipping_zones_script, 'wc_region_picker_update' ) && false === strpos( $shipping_zones_script, 'CustomEvent' ) && false === strpos( $shipping_zones_script, 'coreLocations.concat' ), 'Shipping Zone selection updates remain entirely native with no combined-event rebroadcast' );
assert_address_contract( true, false === strpos( $shipping_zones_source, "\$dependencies[] = 'wc-enhanced-select'" ) && false === strpos( $shipping_zones_source, "\$dependencies[] = 'wc-shipping-zone-methods'" ) && false === strpos( $shipping_zones_source, "\$dependencies[] = 'wc-shipping-zones'" ), 'Shipping Zone assets require no adjacent SelectWoo control and do not enqueue core scripts before WooCommerce localizes them' );
assert_address_contract( true, false !== strpos( $shipping_zones_source, 'addressData.provinces' ) && false !== strpos( $shipping_zones_source, 'addressData.wards' ) && false === strpos( $shipping_zones_source, "'provinces'    =>" ), 'Shipping Zone native tree reuses the existing localized address dataset without a duplicate ward payload' );
assert_address_contract( true, false !== strpos( $shipping_zones_source, "add_filter( 'woocommerce_states'" ) && false !== strpos( $shipping_zones_source, 'add_vietnam_shipping_zone_provinces' ) && false !== strpos( $shipping_zones_source, 'is_shipping_settings_request()' ), 'Shipping settings retain native Vietnamese state nodes independently from the optional Address Fields feature' );
assert_address_contract( true, false === strpos( $shipping_zones_script, '.vck-shipping-zone-wards' ) && false === strpos( $shipping_zones_style, '.vck-shipping-zone-wards' ) && false !== strpos( $shipping_zones_script, 'MutationObserver' ), 'No adjacent ward selector or editor-only style remains, while zone-list enhancements survive a native rerender' );
...
assert_address_contract( true, false !== strpos( $shipping_zones_style, '.vck-shipping-zone-ward-labels' ), 'Shipping Zone list-summary style remains present' );

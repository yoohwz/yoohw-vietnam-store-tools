...
The Human owns product direction, unresolved trade-offs, Merge, version/release decisions, production actions, and WordPress.org publication. ChatGPT owns task framing, genuine Plan Review decisions, and external Acceptance Review; it never implements repository source. Codex owns discovery, implementation, validation, corrections, PR upkeep, and Technical Review orchestration. GitHub is the durable evidence record. Tool access does not grant a role another role's authority.
...
| `Review VST-N` | Manual fresh Technical Review fallback when native independent delegation is unavailable. |

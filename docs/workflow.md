...
A governed implementation PR should include a same-Issue closing link such as `Closes #N`. After a Human-authorized merge has actually succeeded, the agent performing finalization verifies that the canonical task Issue is closed as completed; if GitHub did not close it automatically, close that Issue explicitly. Do not report `FINALIZED` while the task Issue remains open, and do not close it before the merge succeeds. This verification applies even when the PR targets a retained `release/*` branch, where automatic closing may differ. Historical merged tasks are not reopened or rewritten. For VST-47, retain its admitted current-workflow merge gate and close Issue #47 only after its Human-authorized merge succeeds.
